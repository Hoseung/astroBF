...
def get_simple_psf(sise, sigma_psf):
...
def make_figure(morph, nrows=2, savefig=None, img_org=None, norm='log'):
    """
    ...
    """
    if norm == 'log':
        normalizer = lambda image : simple_norm(image, stretch='log', log_a=10000)
    elif norm == "linear":
        normalizer = lambda image : simple_norm(image, stretch='linear')
    ...
    if nrows == 3:        
        # Sersic Model
        ax = _get_ax(fig, 2, 0, nrows, ncols, wpanel, hpanel, htop, eps, wfig, hfig)
        if img_org is None:
            img_org = morph._image
        ax.imshow(np.arcsinh(img_org/0.1), origin='lower')
        ax.scatter(morph.xc_centroid,morph.yc_centroid, marker='*')
        circle_r20 = patches.Circle((morph.xc_centroid, morph.yc_centroid), 
                                        radius=morph.r20,
                                        fill=False)
        circle_r80 = patches.Circle((morph.xc_centroid, morph.yc_centroid), 
                                        radius=morph.r80,color='b',
                                        fill=False)
        ax.add_collection(PatchCollection([circle_r20,circle_r80], match_original=True))
        ...
        fitted_model = statmorph.ConvolvedSersic2D(
                ...
                r_eff=morph.sersic_rhalf, n=morph.sersic_n,
                x_0=morph.sersic_xc, y_0=morph.sersic_yc,
                ellip=morph.sersic_ellip,theta=morph.sersic_theta)

        # Default PSF?
        psf = get_simple_psf(20, 2.0)
        snp=100.0
        ...
        ax = _get_ax(fig, 2, 1, nrows, ncols, wpanel, hpanel, htop, eps, wfig, hfig)
        ax.imshow(image, cmap='gray', origin='lower',
                norm=normalizer(image))
        ax.set_title('Original image')
        
        ax = _get_ax(fig, 2, 2, nrows, ncols, wpanel, hpanel, htop, eps, wfig, hfig)
        ax.imshow(image_model + bg_noise, cmap='gray', origin='lower',
                norm=normalizer(image))
        ax.set_title('Fitted model')

        ax = _get_ax(fig, 2, 3, nrows, ncols, wpanel, hpanel, htop, eps, wfig, hfig)
